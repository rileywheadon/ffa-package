...
validate_rfpl <- function(
  ...
  slices = "last"
	...
	results <- rfpl.uncertainty(df$max, df$year, distr, signature, slices = slices)[[1]]
...
validate_rfgpl <- function(
  ...
  slices = "last"
	...
	results <- rfpl.uncertainty(df$max, df$year, distr, signature, slices = slices, prior = c(6, 9))[[1]]
