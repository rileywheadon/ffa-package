test_that("Test pettitt-test.R on data set #1", {
	...
	expect_equal(test$k.critical, 731.467, tol = 1e-4)
	expect_equal(test$p.value, 0.461, tol = 1e-4)
...
test_that("Test pettitt-test.R on data set #2", {
	...
	expect_equal(test$k.critical, 616.753, tol = 1e-4)
...
test_that("Test pettitt-test.R on data set #3.1", {
	...
	expect_equal(test$k.critical, 807.790, tol = 1e-4)
	expect_equal(test$p.value, 0.022, tol = 1e-4)
...
test_that("Test pettitt-test.R on data set #3.2", {
	...
	expect_equal(test$k.critical, 596.605, tol = 1e-4)
	expect_equal(test$p.value, 0.071, tol = 1e-4)
	expect_equal(test$change.index, 0)
	expect_equal(test$change.year, 0)
...
test_that("Test pettitt-test.R on data set #3.3", {
	...
	expect_equal(test$k.critical, 678.517, tol = 1e-4)
	expect_equal(test$p.value, 0, tol = 1e-4)
