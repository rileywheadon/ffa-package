...
test_that("plot-sffa-assessment.R works on OKANAGAN RIVER (08NM050)", {
	...
	df <- data_local("CAN-08NM050.csv")
	assessment <- model_assessment(df$max, "GEV", "MLE")

	# Test plotting function
	p <- plot_sffa_assessment(assessment)
	expect_s3_class(p, "ggplot")
  	vdiffr::expect_doppelganger("assessment-sffa-default", p) 
...
test_that("plot-sffa-assessment.R works on BOW RIVER (05BB001)", {
	...
	df <- data_local("CAN-05BB001.csv")
	assessment <- model_assessment(df$max, "GEV", "MLE", NULL, df$year, S10)

	# Test plotting function
	p <- plot_nsffa_assessment(assessment)
	expect_s3_class(p, "ggplot")
  	vdiffr::expect_doppelganger("assessment-nsffa-default", p) 

	# Test with custom arguments
	p <- plot_nsffa_assessment(assessment, title = "Title", xlabel = "X", ylabel = "Y")
	expect_s3_class(p, "ggplot")
  	vdiffr::expect_doppelganger("assessment-nsffa-custom", p) 
