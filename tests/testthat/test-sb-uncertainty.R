# NOTE: This test is commented out because it takes a very long time
test_that("Test s-bootstrap.R on data set #1 with non-stationary MLE", {
	...
	# GEV100 <- sb.uncertainty(df, "GEV100", "MLE", years = "first")[[1]]
	...
	# GEV100 <- sb.uncertainty(df, "GEV100", "MLE", years = "last")[[1]]
...
test_that("Test s-bootstrap.R on data set #1 with L-moments", {
	...
	start <- Sys.time()
	GEV <- sb.uncertainty(df, "GEV", "L-moments")[[1]]
	end <- Sys.time()
	print(end - start)
	...
	# Test non-stationary distribution
	# GEV <- sb.uncertainty(df, "GEV100", "MLE", slices = c(1900, 1950, 2000))

	# Gumbel (GUM) Distribution
	GUM <- sb.uncertainty(df, "GUM", "L-moments")[[1]]
	...
	NOR <- sb.uncertainty(df, "NOR", "L-moments")[[1]]
	...
	LNO <- sb.uncertainty(df, "LNO", "L-moments")[[1]]
	...
	GLO <- sb.uncertainty(df, "GLO", "L-moments")[[1]]
	...
	PE3 <- sb.uncertainty(df, "PE3", "L-moments")[[1]]
	...
	GNO <- sb.uncertainty(df, "GNO", "L-moments")[[1]]
	...
	WEI <- sb.uncertainty(df, "WEI", "L-moments")[[1]]
...
test_that("Test s-bootstrap.R on data set #2 with L-moments", {
	...
	# Log-Pearson Type III (LP3) Distribution
	LP3 <- sb.uncertainty(df, "LP3", "L-moments")[[1]]
...
test_that("Test s-bootstrap.R on data set #3.1 with MLE", {
	...
	# Generalized Extreme Value (GEV) Distribution
	GEV <- sb.uncertainty(df, "GEV", "MLE")[[1]]
	...
	# Gumbel (GUM) Distribution
	GUM <- sb.uncertainty(df, "GUM", "MLE")[[1]]
	...
	# Normal (NOR) Distribution
	NOR <- sb.uncertainty(df, "NOR", "MLE")[[1]]
	...
	# Log-Normal (LNO) Distribution
	LNO <- sb.uncertainty(df, "LNO", "MLE")[[1]]
	...
	GLO <- sb.uncertainty(df, "GLO", "MLE")[[1]]
	# print(GLO)
	...
	# Log-Pearson Type III (LP3) Distribution
	LP3 <- sb.uncertainty(df, "LP3", "MLE")[[1]]
	...
	# Generalized Normal (GNO) Distribution
	GNO <- sb.uncertainty(df, "GNO", "MLE")[[1]]
	...
	WEI <- sb.uncertainty(df, "WEI", "MLE")[[1]]
	# print(WEI)
