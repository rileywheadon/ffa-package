...
test_that("Test s-bootstrap.R on data set #1 with L-moments", {
	...
	GUM <- uncertainty_bootstrap(df$max, "GUM", "L-moments")[[1]]
	...
	LNO <- uncertainty_bootstrap(df$max, "LNO", "L-moments")[[1]]
	...
	GLO <- uncertainty_bootstrap(df$max, "GLO", "L-moments")[[1]]
	...
	PE3 <- uncertainty_bootstrap(df$max, "PE3", "L-moments")[[1]]
	...
	GNO <- uncertainty_bootstrap(df$max, "GNO", "L-moments")[[1]]
	...
	WEI <- uncertainty_bootstrap(df$max, "WEI", "L-moments")[[1]]
...
test_that("Test s-bootstrap.R on data set #2 with L-moments", {
	...
	LP3 <- uncertainty_bootstrap(df$max, "LP3", "L-moments")[[1]]
...
test_that("Test s-bootstrap.R on data set #3.1 with MLE", {
	...
	# Gumbel (GUM) Distribution
	GUM <- uncertainty_bootstrap(df$max, "GUM", "MLE", samples = 1000L)[[1]]
	...
	# Normal (NOR) Distribution
	NOR <- uncertainty_bootstrap(df$max, "NOR", "MLE", samples = 1000L)[[1]]
	...
	# Log-Normal (LNO) Distribution
	LNO <- uncertainty_bootstrap(df$max, "LNO", "MLE", samples = 1000L)[[1]]
	...
	# Log-Pearson Type III (LP3) Distribution
	LP3 <- uncertainty_bootstrap(df$max, "LP3", "MLE", samples = 1000L)[[1]]
	...
	# Generalized Normal (GNO) Distribution
	GNO <- uncertainty_bootstrap(df$max, "GNO", "MLE", samples = 1000L)[[1]]
