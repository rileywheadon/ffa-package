...
test_that("basic functionality is working for CRAN", {
	...
	GEV <- uncertainty_bootstrap(df$max, "GEV", "L-moments")$results
	...
	GUM <- uncertainty_bootstrap(df$max, "GUM", "L-moments")$results
...
test_that("uncertainty-bootstrap.R works with L-moments", {
	...
	NOR <- uncertainty_bootstrap(df$max, "NOR", "L-moments")$results
	...
	LNO <- uncertainty_bootstrap(df$max, "LNO", "L-moments")$results
	...
	GLO <- uncertainty_bootstrap(df$max, "GLO", "L-moments")$results
	...
	PE3 <- uncertainty_bootstrap(df$max, "PE3", "L-moments")$results
	...
	GNO <- uncertainty_bootstrap(df$max, "GNO", "L-moments")$results
	...
	WEI <- uncertainty_bootstrap(df$max, "WEI", "L-moments")$results
	...
	LP3 <- uncertainty_bootstrap(df$max, "LP3", "L-moments")$results
...
test_that("uncertainty-bootstrap.R works on BOW RIVER (05BB001) with MLE", {
	...
	# Generalized Extreme Value (GEV) Distribution
	GEV <- uncertainty_bootstrap(df$max, "GEV", "MLE", samples = 1000L)$results
	...
	# Gumbel (GUM) Distribution
	GUM <- uncertainty_bootstrap(df$max, "GUM", "MLE", samples = 1000L)$results
	...
	# Normal (NOR) Distribution
	NOR <- uncertainty_bootstrap(df$max, "NOR", "MLE", samples = 1000L)$results
	...
	# Log-Normal (LNO) Distribution
	LNO <- uncertainty_bootstrap(df$max, "LNO", "MLE", samples = 1000L)$results
	...
	# Log-Pearson Type III (LP3) Distribution
	LP3 <- uncertainty_bootstrap(df$max, "LP3", "MLE", samples = 1000L)$results
	...
	# Generalized Normal (GNO) Distribution
	GNO <- uncertainty_bootstrap(df$max, "GNO", "MLE", samples = 1000L)$results
...
test_that("uncertainty-bootstrap.R works on ATHABASCA RIVER (07BE001) with non-stationary MLE", {
	...
	expect_equal(GEV$results[[1]]$ci_lower , c(1494, 2205, 2665, 3067, 3530, 3848), tol = 1e-2)
	expect_equal(GEV$results[[1]]$estimates, c(1754, 2532, 3078, 3626, 4372, 4960), tol = 1e-2)
	expect_equal(GEV$results[[1]]$ci_upper , c(2045, 2861, 3497, 4234, 5420, 6483), tol = 1e-2)
	
	expect_equal(GEV$results[[2]]$ci_lower , c(1706, 2414, 2860, 3267, 3734, 4049), tol = 1e-2)
	expect_equal(GEV$results[[2]]$estimates, c(1963, 2741, 3287, 3835, 4581, 5169), tol = 1e-2)
	expect_equal(GEV$results[[2]]$ci_upper , c(2224, 3051, 3689, 4434, 5624, 6686), tol = 1e-2)
...
test_that("uncertainty-bootstrap.R works on ATHABASCA RIVER (07BE001) with GMLE", {
	...
	expect_equal(GEV$results[[1]]$ci_lower , c(1484, 2223, 2743, 3285, 4071, 4697), tol = 1e-2)
	expect_equal(GEV$results[[1]]$estimates, c(1730, 2544, 3145, 3771, 4660, 5392), tol = 1e-2)
	expect_equal(GEV$results[[1]]$ci_upper , c(2023, 2922, 3575, 4286, 5309, 6148), tol = 1e-2)

	expect_equal(GEV$results[[2]]$ci_lower , c(1703, 2423, 2954, 3507, 4277, 4887), tol = 1e-2)
	expect_equal(GEV$results[[2]]$estimates, c(1960, 2775, 3375, 4001, 4891, 5622), tol = 1e-2)
	expect_equal(GEV$results[[2]]$ci_upper , c(2239, 3119, 3792, 4510, 5520, 6350), tol = 1e-2)
